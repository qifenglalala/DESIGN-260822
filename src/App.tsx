...
export default function App() {
  ...
  const mobileVideoPlayback = typeof window !== 'undefined' && window.matchMedia('(max-width: 800px)').matches
    ...
    <section className="video-collection" id="contact"><div className="shell">
      ...
      <div className="video-layout">
        ...
        <div className="video-player-card embedded-video">
          ...
          {isVideoPlaying ? <div className="video-embed" style={{ aspectRatio: activeVideoAspectRatio }}><iframe key={activeVideo.url} src={`${activeVideo.url}&autoplay=1&muted=${mobileVideoPlayback ? '1' : '0'}&playsinline=1`} title={`${activeVideo.title}视频播放`} allow="autoplay; fullscreen; picture-in-picture; encrypted-media" allowFullScreen /></div> : <button className={`video-cover ${(videoCategoryIndex===2&&videoItemIndex>0)||('portrait' in activeVideo&&activeVideo.portrait)?'portrait-cover':''}`} style={{ aspectRatio: activeVideoAspectRatio }} onClick={()=>setIsVideoPlaying(true)} aria-label={`播放${activeVideo.title}`}><img src={activeVideo.cover} alt={`${activeVideo.title}视频封面`} loading="lazy" decoding="async"/><span><Play fill="currentColor" size={22}/></span><b>点击播放</b></button>}
        </div>
